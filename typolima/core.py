...
def fix_text(text: str, rules: Dict[str, Any]) -> str:
    ...
    if preps:
        pat = r"\b(" + "|".join(re.escape(p) for p in preps) + r") +(\S)"
    ...
    before = punc.get("space_before", [])
    after = punc.get("space_after", [])

    if before:
        pat_before = r"(\S)\s*(" + "|".join(re.escape(c) for c in before) + r")"
        text = re.sub(pat_before, rf"\1{UNICODE_NBSP}\2", text)

    if after:
        pat_after = r"(" + "|".join(re.escape(c) for c in after) + r")\s*(\S)"
        text = re.sub(pat_after, rf"\1{UNICODE_NBSP}\2", text)

    # ──────────────────────────────────────
    # 5. Dashes & ellipsis – simple
    # ──────────────────────────────────────
    ...
    # Cleanup spaces inside smart quotes if they were added
    if q.get("primary"):
        o, c = q["primary"]
    ...
    # ──────────────────────────────────────
    # 7. Aggressive mode – optional symbols
    # ──────────────────────────────────────
...
def main():
    ...
    parser.add_argument("--lang", required=True, help="language code: cs, sk, pl, en, fr, de, it, es, pt, uk, hu, ro, nl, bg, vi, tr, ru, el, fi, sv, hr, da, no, sl, et, ca, id, tl, sw, eo")
